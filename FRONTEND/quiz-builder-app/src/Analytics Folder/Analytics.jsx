...
const Analytics = () => {
  ...
  const handleShare = (quiz) => {
    ...
    if (quiz.title.toLowerCase() === "poll quiz") {
      quizLink = `http://localhost:3000/quiz/${quiz._id}`;
    } else if (quiz.title.toLowerCase() === "q&a quiz") {
      quizLink = `http://localhost:3000/QA-quiz/${quiz._id}`;
    } else {
      quizLink = `http://localhost:3000/quiz/${quiz._id}`; // Default link for other quiz types
    }
